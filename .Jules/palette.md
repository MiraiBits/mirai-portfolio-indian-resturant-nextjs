...
## 2025-02-14 - Modal Focus Timing
**Learning:** When conditionally rendering a modal in React, calling `.focus()` immediately inside `useEffect` may fail if the browser hasn't painted the element yet.
**Action:** Use `requestAnimationFrame(() => ref.current?.focus())` to ensure the element is in the DOM and ready to receive focus.
